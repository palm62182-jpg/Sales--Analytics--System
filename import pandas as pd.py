...
# 1. CREATE THE DATA FILE (So you don't have to do it manually)
raw_data = """TransactionID|Date|ProductID|ProductName|Quantity|UnitPrice|CustomerID|Region
...
with open("sales_data.txt", "w", encoding="latin-1") as f:
    f.write(raw_data)
...
def process_sales_data(file_path):
    ...
    # Handle Non-UTF-8 encoding (latin-1 is common for messy files)
    try:
    ...
    # Clean 'UnitPrice' and 'Quantity' (Remove commas first)
    df['UnitPrice'] = df['UnitPrice'].astype(str).str.replace(',', '').astype(float)
    df['Quantity'] = pd.to_numeric(df['Quantity'], errors='coerce')

    # Data Cleaning Rules:
    # 1. TransactionID must start with 'T'
    invalid_id = ~df['TransactionID'].str.startswith('T', na=False)
    
    # 2. Quantity > 0 and UnitPrice > 0
    invalid_numbers = (df['Quantity'] <= 0) | (df['UnitPrice'] <= 0)
    
    # 3. Missing CustomerID or Region
    missing_fields = df['CustomerID'].isna() | (df['CustomerID'] == '') | \
                     df['Region'].isna() | (df['Region'] == '')

    # Combine filters for Removal
    to_remove = invalid_id | invalid_numbers | missing_fields
    
    # Report Statistics
    invalid_records = df[to_remove]
    valid_df = df[~to_remove].copy()
    
    # 4. Clean ProductNames (Remove commas, keep record)
    valid_df['ProductName'] = valid_df['ProductName'].str.replace(',', ' ')

    print(f"Total records processed: {initial_count}")
    print(f"Records removed (Invalid): {len(invalid_records)}")
    print(f"Records kept (Valid): {len(valid_df)}")
    print("-" * 38)

    # 3. SIMPLE ANALYSIS
    valid_df['TotalSales'] = valid_df['Quantity'] * valid_df['UnitPrice']
    
    print("\n--- Sales Summary by Region ---")
    region_summary = valid_df.groupby('Region')['TotalSales'].sum().sort_values(ascending=False)
    print(region_summary)

    # Export cleaned data
    valid_df.to_csv("cleaned_sales_data.csv", index=False)
    print("\nCleaned data saved to 'cleaned_sales_data.csv'")

if __name__ == "__main__":
    process_sales_data("sales_data.txt")